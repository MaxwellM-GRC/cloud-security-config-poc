...
def test_evidence_is_rcm_ready_and_population_reconciles(tmp_path):
    ...
    assert payload["control"]["control_description"].startswith("Management periodically")
...
def test_one_case_and_one_csv_row_exist_per_finding(tmp_path):
    ...
    assert all(row["remediation"] and row["mitigation"] and row["closure_evidence"] for row in rows)
